...
Future VM types (LuaJITVM, ContainerizedLuaVM) deferred until after MVP validation.
...
### 3. LLM Glue Layer Architecture
Implement LLM-based intelligent middleware that:
...
- Dom0 command translation utilities (LLM-generated middleware code)
...
class LuaVMFactory:
    ...
    def create_vm(vm_type, vm_id, config=None):
        # MVP: Only basic and xen types supported
        vm_classes = {
            "basic": BasicLuaVM,
            "xen": XenDom0VM  # NEW - MVP implementation only
        }
...
1. Implement XenDom0VM class with LLM glue layer
...
- LLM glue layer successfully translates between Python API and Dom0 operations
